      ...
      <section className="relative mx-auto max-w-[1080px] px-[var(--space-6)] pb-20 md:pb-28">
        <div>
          {events.map((event, index) => {
            ...
            return (
              <article key={event.id} className="grid gap-8 border-t border-[color:var(--c-deep-ocean)]/15 py-12 md:grid-cols-[0.9fr_1.1fr] md:items-center md:gap-16 md:py-16">
                <Link href={`/events/${event.slug}`} aria-label={`${event.title}の詳細を見る`} className={index % 2 ? "md:order-2" : undefined}>
                  <Image src={image.sourceUrl} alt={image.altText || `${event.title}の写真`} width={1200} height={800} className="aspect-[3/2] w-full rounded-[var(--radius-2xl)] object-cover transition-transform duration-500 hover:scale-[1.015]" />
                </Link>
                <div>
                  ...
                  <p className="mt-5 text-lg font-black text-[color:var(--c-deep-ocean)]">{period || "開催日調整中"}</p>
                  <h2 className="mt-3 text-3xl font-black leading-tight tracking-[-0.02em] text-[color:var(--c-deep-ocean)]">{event.title}</h2>
                  {fields?.catchCopy ? <p className="mt-4 leading-7 text-[color:var(--c-text-secondary)]">{fields.catchCopy}</p> : null}
                  {fields?.venueName ? <p className="mt-4 text-sm font-bold text-[color:var(--c-text-secondary)]">会場　{fields.venueName}</p> : null}
                  <Link href={`/events/${event.slug}`} className="mt-6 inline-flex min-h-11 items-center border-b border-[color:var(--c-deep-ocean)] pb-1 font-black text-[color:var(--c-deep-ocean)]">開催情報を見る →</Link>
                </div>
              </article>
            );
          })}
