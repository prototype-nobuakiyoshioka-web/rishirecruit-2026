...
- [ ] **Phase 2: 3Dアセット制作** — パステル調ミニチュア版を作成・組み込み済み（2026-09-14）。実機性能・最終デザイン確認は継続
  - [x] 山腹の家（利尻岳山小屋、OSM way 515743750）をPC/SPのGLB・編集用Blender・生成処理から除外（2026-09-25）。一般建物67棟。地形・道路・ランドマーク等の圧縮メッシュが変更前と一致することと、地理配置検証・型チェック・対象ESLintの通過を確認。詳細は `docs/07-island-miniature.md`。
...
- [x] **Phase 4: フロントエンド基盤** — 完了
  ...
  - [x] 共通CTAをブルー系のおもちゃプレートへ統一（2026-09-25）。求人一覧・固定応募・フォーム送信に角丸、内枠、ハイライト、厚みと押下表現を適用。矢印・封筒・送信中表示、フォーカスと動きを減らす設定に対応。型チェック・対象ESLint・本番ビルド通過、PC/320px/390px幅の代表ページ表示と一覧遷移・応募フォームへの移動を確認（フォーム実送信は未実施）。
...
- [ ] **Phase 5: 3Dシーン実装** — 進行中
  ...
  - [x] トップのコラム導線をブルー系のおもちゃプレートに更新（2026-09-25）。利尻山を表紙に描いたミニチュア本、内枠と厚み、矢印、押下表現を追加。PC/390px幅の表示・コラム一覧への遷移・型チェック・対象ESLintを確認。
...
- [ ] **Phase 8: テスト・デプロイ** — 一部着手
  ...
  - [~] Vitest / Playwright・アクセシビリティ・Lighthouse — **テスト基盤導入済み**。Vitest(4.1.11 pin)でユニット29件(`tests/unit/`: format/format-date/grid-spans/seo/turnstile、`@/`はViteネイティブtsconfigPaths、node環境)、Playwright(1.62.1 pin)でE2Eスモーク3件(`tests/e2e/smoke.spec.ts`: robots.txt/sitemap.xml/privacy、WP非依存の静的ルートのみ、webServerで`next dev`自動起動)。スクリプト: `npm run test`/`test:watch`/`test:e2e`。生成物は.gitignore除外済み。全29+3件パス確認。**残: アクセシビリティ監査・Lighthouse計測は未着手。E2Eのフォーム/WP実データ経路のテスト拡充も今後**
  - [ ] 本番ホスティング・環境変数・監視
