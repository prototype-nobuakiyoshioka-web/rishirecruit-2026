...
画像で指定された利尻岳山小屋（OSM way `515743750`）の壁・屋根・窓を、PC/SPのGLBおよび編集用Blenderから削除。生成処理では建物選定後にこの1棟を除外し、他の建物の配置と乱数順を維持した。一般建物は67棟。読み込みURLのバージョンも更新した。

- 現行PC: 101,109三角形 / 1,106,848 bytes、SP: 80,147三角形 / 988,740 bytes（オタトマリ沼追加後のモデルから各262三角形減）。
- 削除前後のGLBを比較し、建物以外の圧縮メッシュデータは完全一致。配置manifestも対象1棟以外は一致。
- 地理配置の既存検証を通過。6グループ・8ランドマーク・テクスチャ0を維持。
